...
begin
	WEIGHTING_METHOD = :pchip  # :hist for histogram based
							   # :pchip for Piecewise Cubic Hermite Interpolating
							   #        Polynomial hermite
	                           # :uniform for uniform weights

	VAL_TEST_MODE = :none  # :sampled from df_raw or
	                       # :year taken from years
						   # :none for using all data for training
...
begin
	...
	MIN_WEIGHT = 0.01  # minimal weight when :hist and :pchip weighting
	                   # method is used
end;
...
# ╔═╡ 1bf0e7d9-0a2d-40b2-9f88-457890f9d640
PATH_DATA = joinpath(path_processed, data_dir)

# ╔═╡ cb63f571-02a6-4be9-9e4a-a5f8e2128dcb
begin
	local dummydf = DataFrame(read_parquet(PATH_DATA))

	target_vars = filter(
				x -> startswith(x, "data_mehs") || startswith(x, "data_poh"),
				names(dummydf)
	)

	local covariables = setdiff(
							names(dummydf),
							[
								target_vars;
								[
...
# ╔═╡ e1c744b0-280f-4c9f-87b2-90d908ed8910
@bind sel_target Select(target_vars)

# ╔═╡ 012c3642-c15b-46a5-8462-e64895157757
md"""
## Weibull fit on target variable
"""

# ╔═╡ e1845fee-85f6-47fd-8f6f-4053ab8886fe
md"""
Export data set
$(@bind export_data CheckBox())
"""

# ╔═╡ 9d5f7b5a-5e6d-4de8-b8cb-0c18816f26be
md"""
## Preparing data
"""

# ╔═╡ 8ee3f969-4399-440b-b131-8c4b92aecb53
begin
	df_raw = DataFrame(read_parquet(PATH_DATA))

	local dates = Dates.Date.(Dates.unix2datetime.(df_raw.timestamp))

	df_transformed = df_raw[:, proxies]
	df_transformed.target = df_raw[:, Symbol(sel_target)]

	df_transformed.breite = df_raw.lat / 1_000_000
	df_transformed.laenge = df_raw.lon / 1_000_000

	df_transformed.year = Dates.value.(Dates.Year.(dates))
	df_transformed.yday = Dates.dayofyear.(dates)
...
if SHOW_PLOTS
	...
	Plots.histogram(sampled_target, bins=min_sample:max_sample)
end
...
if SHOW_PLOTS
	...
	local sampled_weibull = sort(rand(weibull_fitted_on_target, NR_SAMPLES))

	Plots.plot(sampled_target_plot, 1:NR_SAMPLES, label="target")
	Plots.plot!(sampled_weibull, 1:NR_SAMPLES, label="weibull")
end

# ╔═╡ 13fa3e4b-8e6c-4d60-a7fe-46d06e5f6112
if SHOW_PLOTS
	qqplot(sampled_weibull, sampled_target_plot, qqline=:R)
end

# ╔═╡ f38fb9d6-82a9-4b3c-9a50-96d1591f51e7
if SHOW_PLOTS
	Distributions.params(weibull_fitted_on_target)
end
...
if export_data
	...
	df_full.location_id = groupby(df_full, loc_columns) |> groupindices

	select!(df_full, Not(loc_columns))
...
# ╔═╡ 5cc6f738-951b-44da-abf5-10b47e5edbf2
describe(df_transformed)

# ╔═╡ bfdf4274-aede-4178-a783-4da13b53ab0b
if (VAL_TEST_MODE == :sampled)
	train_df, val_df, test_df = partition(
									df_transformed,
									0.8,
									0.1,
									shuffle=true
)
elseif (VAL_TEST_MODE == :year)
	train_df = filter(
					x -> (x.year >= START_YEAR) && (x.year ∉ VAL_TEST_YEARS),
					df_transformed
	)

	val_df, test_df = partition(
						filter(x -> x.year ∈ VAL_TEST_YEARS, df_transformed),
						0.5,
						shuffle=true
	)
elseif (VAL_TEST_MODE == :none)
	train_df, val_df = partition(
									df_transformed,
									0.9,
									shuffle=true
)
...
# ╔═╡ db1b667e-fcb5-48b1-8c44-ffc6c25cee21
function scaledata(df; fitted_transform=nothing)
	unscaled_data = Matrix(select(df, Not(:target)))'

	t = isnothing(fitted_transform) ? fit(UnitRangeTransform, unscaled_data) : fitted_transform

	return StatsBase.transform(t, unscaled_data), df[!, :target], t
end

# ╔═╡ fefa075e-3f43-4f44-b187-cc41752c5c8f
begin
	X_train, Y_train, transform_train = scaledata(train_df)
	X_val, Y_val, _ = scaledata(val_df; fitted_transform=transform_train)

	train_dataloader = Flux.DataLoader(
								(X_train, Y_train),
								batchsize=BATCHSIZE,
								parallel=true,
								shuffle=true,
	...
	val_dataloader = Flux.DataLoader(
							(X_val, Y_val),
							batchsize=16384,
							parallel=true,
	)

	if (VAL_TEST_MODE != :none)
		X_test, Y_test, _ = scaledata(test_df; fitted_transform=transform_train)

		test_dataloader = Flux.DataLoader(
								(X_test, Y_test),
								batchsize=16384,
								parallel=true,
		)
...
# ╔═╡ f0811139-6070-4a21-bac3-42bf8aa8b79d
md"""
## Model
"""

# ╔═╡ 41fa865b-1410-444a-b70f-0032158295fb
begin
	local target_counter = StatsBase.countmap(df_transformed.target)
	local maxocc = maximum(values(target_counter))

	target_weights::Dict{Float32, Float32} = Dict()

	if WEIGHTING_METHOD == :uniform
		for k in keys(target_counter)
			target_weights[k] = 1
		end
	elseif WEIGHTING_METHOD == :hist
		for k in keys(target_counter)
			target_weights[k] = 1 + MIN_WEIGHT - (target_counter[k] / maxocc)
		end
	elseif WEIGHTING_METHOD == :pchip
		...
		# precalculations to get smallest hailsize weight depending on relative
		# occurrence
		smallest_hail = minimum(keys(target_counter))
		largest_hail = maximum(keys(target_counter))

		# init with smallest weight depending on relative occurrence
		xs = [smallest_hail]
		ys = [MIN_WEIGHT]

		# Compute weight of every percentile depending on relative occurrence
		for perc in 0:100
			hs = percentile(df_transformed.target, perc)

			# The same hs could occur multiple times, so immediately
		...
		# Interpolate for every hail size present in the labels
		for k in keys(target_counter)
...
# ╔═╡ c6127e3e-b414-47ea-9d78-a59bad3093ed
if SHOW_PLOTS
	Plots.scatter(
		collect(keys(target_weights)),
		collect(values(target_weights)),
		title="Weight plot"
	)
end;

# ╔═╡ 99ef26fb-471d-4188-966c-3e93b081d875
begin
	# The model outputs shape and scale.
	# For improved readability we define the  indices here.
	SHAPE_INDEX = 1  # shape parameter w or α
	SCALE_INDEX = 2  # scale parameter C or θ
end;
...
begin
	...
	function loglike_wb(ŷ, y, agg)
		...
		return agg(-weight .* logpdf.(ŷ, y))  # TODO: logpdf is the same as loglikelihood
                                              #       applied pointwise. Verify and use
                                              #       loglikelihood for improved readability
	end
	...
	function calc_loss(data_loader, model, device)
	    ls = 0.0f0
	    n_samples = 0

	    for (x, y) in data_loader
	        x, y = device(x), device(y)
	        ls += loss(model, x, y; agg=sum)
...
# ╔═╡ b879ab15-11d5-451b-a4f9-07c6c57f18f1
md"""
Start training
$(@bind training_start CheckBox())
"""

# ╔═╡ 223f2258-ed7f-4c97-9245-0592170e99bf
if training_start
	local init_strategy = Flux.kaiming_uniform
    local device = cpu
	...
	if (model_settings[:val_test_mode] == :year)
		model_settings[:val_test_years] = VAL_TEST_YEARS
	end

	if (model_settings[:weighting_method] ∈ [:hist, :pchip])
		model_settings[:min_weight] = MIN_WEIGHT
	end

	open(joinpath(save_path, "model_settings.json"), "w") do f
    	JSON.print(f, model_settings, 4)
	end

	serialize(joinpath(save_path, "transform_train.jls"), transform_train)
	write_parquet(joinpath(save_path, "dataframe.parquet"), df_transformed)

	model = Chain(
  	  Chain(
	      Dense(
			  model_settings[:num_covariables] => model_settings[:nodes_per_layer],
			  init=init_strategy
		  ),
  	  	  BatchNorm(model_settings[:nodes_per_layer], ACTIVATION_FUNCTION),
	  ),
	  [
		  Chain(
		    Dense(
		  		model_settings[:nodes_per_layer] => model_settings[:nodes_per_layer],
		  		init=init_strategy
	  		),
  	  		BatchNorm(model_settings[:nodes_per_layer], ACTIVATION_FUNCTION),
	  	  )
		  for _ in 2:model_settings[:nr_hidden_layers]
	  ]...,
      Dense(model_settings[:nodes_per_layer] => 2, softplus, init=init_strategy),
	) |> device
	...
	es = Flux.early_stopping(
				identity,
				model_settings[:early_stopping_patience];
				init_score=1e6,
	)
	...
	append!(train_loss, calc_loss(train_dataloader, model, device))
	append!(val_loss, calc_loss(val_dataloader, model, device))

	if (VAL_TEST_MODE != :none)
		test_loss = Float32[]
		append!(test_loss, calc_loss(test_dataloader, model, device))
	end
	...
	for epoch in 1:model_settings[:nr_epochs]
		Flux.train!(loss, model, train_dataloader, opt)

		append!(train_loss, calc_loss(train_dataloader, model, device))
		append!(val_loss, calc_loss(val_dataloader, model, device))

		epochstr = lpad(epoch, 3, "0")
		@save joinpath(save_path, "model_$(epochstr).bson") model
		...
		if (VAL_TEST_MODE != :none)
			append!(test_loss, calc_loss(test_dataloader, model, device))
			df_to_store[!, :test_loss] = test_loss
		end

		CSV.write(
				joinpath(save_path, "loss.csv"),
				df_to_store,
		)

        @info "Epoch = $epoch - train_loss = $(train_loss[end]) - val_loss = $(val_loss[end])" * ((VAL_TEST_MODE != :none) ? " test_loss = $(test_loss[end])" : "")

		if val_loss[end] < bestvalloss
			@info "Val loss decreased from formerly best $(bestvalloss) to $(val_loss[end])"

			global bestmodel = deepcopy(model)
			global bestvalloss = val_loss[end]
			global bestepoch = epoch
		end

        if es(val_loss[end])
			@info "Early Stopping patience $(model_settings[:early_stopping_patience]) reached. Best model reached after $(bestepoch) epochs. Aborting and saving best model."
			break
		end
	end

	@save joinpath(save_path, "best_model.bson") bestmodel
end;

# ╔═╡ e2be26ee-7ae7-4578-aa5b-a2ad5e488722
md"""
## Plotting Diagnostics
"""

# ╔═╡ 20c652e8-c4ee-474e-b585-92ecbba190c1
if training_start
	...
	Plots.savefig(plot, joinpath(save_path, "loss_plot.html"))

	plot
end

# ╔═╡ 0c556a40-80f9-4f34-9d0a-c8ee8b6a5d23
if training_start
	local X = (VAL_TEST_MODE == :none) ? X_val : X_test

	params_weibull = bestmodel(X)

	dist_weibull = Weibull.(
						params_weibull[SHAPE_INDEX, :],
						params_weibull[SCALE_INDEX, :]
	)

	# each row carries the expected value of the weibull distribution
	# for a specific sample
	pred = mean.(dist_weibull)

	sample_idx = sample(1:length(pred), NR_SAMPLES)
end;

# ╔═╡ 3b4a6b2a-60f2-4cdf-a824-f1cfe4bb45b3
if training_start
	local Y = (VAL_TEST_MODE == :none) ? Y_val : Y_test

	local plot = marginalkde(pred[sample_idx], Y[sample_idx])

	local fn = (VAL_TEST_MODE == :none) ? "pred_vs_val.html" : "pred_vs_test.html"

	Plots.savefig(plot, joinpath(save_path, fn))

	plot
end

# ╔═╡ 3df7fff3-192a-4244-82d6-5150942e7de3
if training_start
	local Y = (VAL_TEST_MODE == :none) ? Y_val : Y_test

	local prob = cdf.(dist_weibull[sample_idx], Y[sample_idx])

	local qres = Distributions.quantile.(Distributions.Normal(), prob)

	local plot = Plots.histogram(qres)

	local fn = let
			if (VAL_TEST_MODE == :none)
				"qres_histogramm_val.html"
			else
				"qres_histogramm.html"
			end
	end
...
# ╔═╡ deb81feb-70a2-4e5f-86a1-72c09da0c6cb
if training_start
	local Y = (VAL_TEST_MODE == :none) ? Y_val : Y_test

	res = pred[sample_idx] - Y[sample_idx]

	local plot = Plots.histogram(res, normalize=:pdf, bins=100)

	local fn = let
			if (VAL_TEST_MODE == :none)
				"res_histogramm_val.html"
			else
				"res_histogramm.html"
			end
	end
...
# ╔═╡ f7fd7690-c4b9-458d-8290-20ae45e528ec
if training_start
	local Y = (VAL_TEST_MODE == :none) ? Y_val : Y_test

	local plot = marginalkde(Y[sample_idx], res)

	local fn = let
			if (VAL_TEST_MODE == :none)
				"res_vs_size_kde_val.html"
			else
				"res_vs_size_kde.html"
			end
	end
...
manifest_format = "2.0"
project_hash = "17f2798867bae51fdc234c6adfd4ab73ceb9c571"
...
[[deps.PCHIPInterpolation]]
git-tree-sha1 = "fa9c9839789b203e99580cd09c212df78f883562"
uuid = "afe20452-48d1-4729-9a8b-50fb251f06cd"
version = "0.2.1"
weakdeps = ["RecipesBase"]

    [deps.PCHIPInterpolation.extensions]
    PCHIPInterpolationRecipesBaseExt = "RecipesBase"
...
git-tree-sha1 = "bc24b4dc9ec4b34bca36659ae7efae9085442d80"
...
[[deps.snappy_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "8bb12ecfc3ffde2ef72a957cf2fab27cc193d251"
