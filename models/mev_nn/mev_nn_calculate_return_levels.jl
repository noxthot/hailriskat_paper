...
function scaledata(df, fitted_transform)
	unscaled_data = Matrix(select(df, Not(:target)))'

	return StatsBase.transform(fitted_transform, unscaled_data), df.target
end

# ╔═╡ 844901f1-2e52-4860-89f6-38eab3ef4277
path_processed = joinpath("..", "..", "data", "models", "mev_nn")
...
md"""
Start Calculation
$(@bind calculation_start CheckBox())
"""

# ╔═╡ 4ef9de65-2dbb-45a6-98ef-65e34e484b0b
md"""
## Return levels
"""

# ╔═╡ 3e0c24a9-edcc-4fa4-b9c8-9b1b12a2948b
md"""
## UI setup
"""

# ╔═╡ 350ed3ca-335e-4c89-b1cf-5cd2db1250f5
sel_train_cyc = @bind model_dir Select(sort(readdir(path_processed), rev=true));

# ╔═╡ 30d11131-d107-4bf8-a575-fbaf5d588304
modelpath = joinpath(path_processed, model_dir)

# ╔═╡ 2c704a6d-1c5d-4ceb-80d0-a910ba6da092
begin
	local modelsettings = JSON.parsefile(joinpath(modelpath, "model_settings.json"))

	use_censored_weibull = get(modelsettings, "censored_weibull", false)
end;

# ╔═╡ 55e9bf22-e35b-47d0-b389-3163c040180c
begin
	function ptmev_single(
				one_location_years::SubArray{T, 1, Vector{T}},
				one_location_weibull_dist::Vector,
				x::Float64
	) where {T<:AbstractFloat}
		# Pointwise probability that the target variable (e.g. MEHS) takes on a value # smaller than x
		df_pointwise_prob = DataFrame(
					year=one_location_years,
					ret1=cdf.(one_location_weibull_dist, x)
		)

		# Yearwise probability that the target variable takes on a value
		# smaller than x
		df_yearwise_prob = combine(groupby(df_pointwise_prob, :year), :ret1 => prod)

		# Average probability that the target value is smaller than x in one-year
		# observation window
		df_average_prob = mean(df_yearwise_prob.ret1_prod)

	  	return df_average_prob
	end


	function qtmev_single(data_one_location, returnquantile)
		wbdist = let
					wb = Weibull.(
									data_one_location.w,
									data_one_location.C
					...
					use_censored_weibull ? censored.(wb,	1.0f0, 88.0f0) : wb
		end

		minfun = (
			x -> ptmev_single(data_one_location.year, wbdist, x) .- returnquantile
		)

		try
			return find_zero(minfun, (0, use_censored_weibull ? 80 : 200))
		catch
			return missing
		end
	end
...
# ╔═╡ 0804405f-26f4-4e60-b2ae-549cbcd80d9e
sel_model = @bind modelname Select(
								filter(
									x -> endswith(x, ".bson"),
									readdir(modelpath)
								),
...
# ╔═╡ 35defbcb-594a-4e3b-bb61-67df693e3e42
begin
	transform_train = deserialize(joinpath(modelpath, "transform_train.jls"))

	local df_raw = DataFrame(read_parquet(joinpath(modelpath, "dataframe.parquet")))

	df = dropmissing(df_raw)

	@assert nrow(df) == nrow(df_raw)

	if occursin("best_model", modelname)
		BSON.@load joinpath(modelpath, modelname) bestmodel
		model = bestmodel
	else
		BSON.@load joinpath(modelpath, modelname) model
	end
end

# ╔═╡ f1599c1a-7f5b-4939-8322-8fc0baeca6b9
if calculation_start
	local paras_matrix = model(scaledata(df, transform_train)[1])'

	local paras_df = DataFrame(paras_matrix, ["w", "C"])  # w = shape; C = scale

	final_df = hcat(
				select(df, ["target", "breite", "laenge", "year", "yday"]),
				paras_df
	)
end

# ╔═╡ 1ce578ea-7740-49b9-bd98-c8084573c9b0
if calculation_start
	describe(final_df)
end

# ╔═╡ 85935a7c-7e33-11ee-3c90-913b0d383c72
if calculation_start
	write_parquet(joinpath(modelpath, "parameters_$(modelname).parquet"), final_df)
end;

# ╔═╡ e10b0a8e-d579-4f95-a2c6-cef039888188
sel_returnlevel = @bind returnyear_mul MultiSelect([[2, 3]; 5:5:100], default=[2, 5, 10, 20, 30, 50]);

# ╔═╡ 3a9e4b85-12d5-4686-a311-c9d391babbe6
if calculation_start
	local returnquantile_mul = 1 .- 1 ./ returnyear_mul

	for (returnquantile, returnyear) in zip(returnquantile_mul, returnyear_mul)
		@info "Computing return period $(returnyear)"

		df_grouped_by_location = groupby(final_df, [:laenge, :breite])
		qtmevbroadcast = (df_onegroup -> qtmev_single(df_onegroup, returnquantile))

		returns_df = combine(df_grouped_by_location, qtmevbroadcast, threads=true)
		rename!(returns_df, :laenge => :lon, :breite => :lat, :x1 => :target)

		CSV.write(
			joinpath(modelpath, "returns_y$(returnyear)_$(modelname).csv"),
			returns_df
		)

		@info "Creating histogram for $(returnyear)"
		plot = Plots.histogram(
					returns_df.target, bins=minimum(skipmissing(returns_df.target)):maximum(skipmissing(returns_df.target))
		)

		Plots.savefig(
			plot,
			joinpath(modelpath, "returnlevel_$(returnyear)y_target_hist.html")
		)
	end
end;

# ╔═╡ 93986aa0-0759-417a-a363-75165348bba9
sidebar = Div([
	@htl("""
	<header>
	...
	"""),
	md""" **Select training cycle:**""",
	sel_train_cyc,
	md"""**Choose model:**""",
	sel_model,
	md"""**Return level period (years):**""",
	sel_returnlevel,
], class="plutoui-sidebar aside")

# ╔═╡ 0400d944-ad13-454a-963e-999a25cd90b9
html"""
<style>
...
manifest_format = "2.0"
project_hash = "904cc4adb6eabd9cc8193b59aab9d3f9e28be4b7"
