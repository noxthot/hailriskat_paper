...
begin
	...
	function hs_single(data_one_location, hs, use_censored_weibull)
		wbdist = let
					wb = Weibull.(
									data_one_location.w,
									data_one_location.C
		...
		return 1 / (1 - ptmev_single(data_one_location.year, wbdist, hs))
	end
end;
...
sel_returnperiod = @bind returnperiod_mul MultiSelect(1:5, default=3:5);
...
if calculation_start
	for model_dir in sort(readdir(path_processed))
		...
		returnperiod_mul_todos = []

		for returnperiod in returnperiod_mul
			returnperiod_fp = joinpath(
								modelpath, "returnperiod_$(returnperiod)cm_$(modelname).csv"
			)

			#!isfile(returnperiod_fp) && push!(returnperiod_mul_todos, returnperiod)
			push!(returnperiod_mul_todos, returnperiod)
		end

		if isempty(returnperiod_mul_todos)
		...
		for hs in returnperiod_mul_todos
			@info "Computing return period for $(hs) in $(model_dir)"

			hs_mm = hs * 10
			df_grouped_by_location = groupby(final_df, [:laenge, :breite])
			hsbroadcast = (df_onegroup -> hs_single(df_onegroup, Float64(hs_mm), use_censored_weibull))

			return_years = combine(
							df_grouped_by_location,
							hsbroadcast,
							threads=true
			)
			...
			CSV.write(
				joinpath(modelpath, "returnperiod_$(hs)cm_$(modelname).csv"),
				return_years
			)

			plot = Plots.histogram(
						return_years.years, bins=0:100
			)

			Plots.savefig(
				plot,
				joinpath(modelpath, "returnperiod_$(hs)cm_target_hist.html")
			)
		end
	end
end
